...
module.exports = class FileTransfer extends EventEmitter {
  constructor (stream, remotePk, options = {}) {
      ...
      onmessage (channel, type, message) {
        ...
        switch (type) {
          ...
          case QUEUED:
            ...
            // TODO get queue number from message
            break
          default:
    ...
    stream.on('data', (data) => {
      ...
      if (!success) self.log('Error on receive')
    })
  }

  // _write (data, enc, cb) {
  //   this.log('******************************************Got data!')
  //   const success = this.smc.recv(data)
  //   if (!success) this.log('Error on receive')
  //   cb()
  // }
  ...
  onData (dataMessage) {
    ...
    if (download.bytesReceived === download.size) {
      ...
      if (download.givenHash === download.sha256) {
        ...
        this.emit('verified', download.sha256)
      } else {
