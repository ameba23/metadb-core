...
const { SHA256_BYTES } = require('../crypto')
const log = require('debug')('metadb-file-transfer-request')
    ...
    pull(
      ...
      pull.asyncMap((fileObj, cb) => {
        ...
        metadb.files.get(fileObj.key, (err, metadata) => {
          if (err) {
            log('Warning: Requested file is not in our database')
