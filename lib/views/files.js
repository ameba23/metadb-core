  ...
  return {
    ...
    map: function (msgs, next) {
      ...
      async function processMsgs () {
        ...
        if (totals.files || totals.bytes) {
          const existingValue = await level.get(TOTALS + 'F')
            .catch(() => { return { files: 0, bytes: 0 } })
          totals.files += existingValue.files
          totals.bytes += existingValue.bytes
          ops.push({
            type: 'put',
            key: TOTALS + 'F',
            value: {
        ...
        for (const holder of Object.keys(totals.holders)) {
          const existingValue = await level.get(TOTALS + 'H!' + holder)
            .catch(() => { return { files: 0, bytes: 0 } })
          totals.holders[holder].files += existingValue.files
          totals.holders[holder].bytes += existingValue.bytes
          ops.push({
            type: 'put',
            key: TOTALS + 'H!' + holder,
            value: {
              files: totals.holders[holder].files,
              bytes: totals.holders[holder].bytes
            }
          })
        }
      }
    ...
    api: {
      ...
      getTotals: async function (kappa) {
        const totals = await level.get(TOTALS + 'F')
          .catch(() => { return { files: 0, bytes: 0 } })
        ...
        for await (const entry of level.createReadStream({
          ...
          holders[entry.key.split('!')[2]] = entry.value
        }

        return Object.assign(totals, { holders })
      },
