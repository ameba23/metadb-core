...
const { nameToWallMessageTopic } = require('../crypto')
      ...
      async function processMsgs () {
        for await (const msg of msgs) {
          ...
          const swarmKey = swarmKeys.find((k) => {
            plain = crypto.secretUnbox(msg.value.private.symmetric, nameToWallMessageTopic(k))
            return plain
          })
          if (!swarmKey) {
            ops.push({
    ...
    api: {
      ...
      updateSwarms: async function (kappa, updatedSwarms, getFeed) {
        ...
        for await (const entry of level.createReadStream({
          ...
          let plain
          const swarmKey = swarmKeys.find((k) => {
            plain = crypto.secretUnbox(message.private.symmetric, nameToWallMessageTopic(k))
            return plain
          })
