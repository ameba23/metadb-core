...
const SWARM_TOPIC_CONTEXT = genericHash(Buffer.from('metadb'), ZERO)
const WALL_MESSAGE_CONTEXT = genericHash(Buffer.from('wall message'), ZERO)
...
module.exports = {
  SIGN_PUBLIC_KEY_BYTES: sodium.crypto_sign_PUBLICKEYBYTES,
  ZERO,
  Sha256,
  genericHash,
  ...
  nameToWallMessageTopic (name) {
    name = Buffer.from(name)
    const topic = genericHash(name, WALL_MESSAGE_CONTEXT)
    return topic
  }
}
