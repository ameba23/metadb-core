...
module.exports = function (metadb) {
  const log = debug(`metadb-swarm ${metadb.keyHex.slice(0, 4)}`)
  class Swarm {
    constructor () {
      ...
      this.swarm.on('connection', (socket, details) => {
        ...
        handshake(metadb.keypair, !isInitiator, transferStream, topics, onPeer, (err, cryptoParams) => {
          if (err) {
            ...
            log('To deduplicate:', metadb.keypair.publicKey.slice(0, 2).toString('hex'), remotePk.slice(0, 4))
            ...
            if (!deduplicated) {
              ...
              log('flushing....')
              self.swarm.flush((err) => {
                if (err) throw err
                log('flush fn called!', transferStream.destroyed)
                if (!transferStream.destroyed) {
                  self.handleReplication(indexStream, isInitiator)

                  metadb.connectedPeers[remotePk] = fileTransfer(metadb)(transferStream, remotePk)
                  transferStream.on('close', () => {
                    log('Transfer stream closed')
                    delete metadb.connectedPeers[remotePk]
    ...
    handleReplication (indexStream, isInitiator) {
      ...
      replicationStream.on('remote-feeds', () => {
        ...
        metadb.core.feeds().forEach((feed) => {
          feed.update(() => {
            log(`Update from feed ${feed.key.slice(0, 2).toString('hex')}`)
          })
          const downloadListener = function (i) {
            log(`downloaded block ${i} from feed ${feed.key.slice(0, 2).toString('hex')}`)
            // downloading = true
          ...
          feed.on('sync', () => {
            ...
            log(`Synced with feed: ${feed.key.slice(0, 2).toString('hex')} new length is ${feed.length}`)
            feed.once('download', downloadListener)
          })
        })
      })
      ...
      pump(
        ...
        (err) => {
          ...
          if (err) log('Replication stream dropped prematurely', err)
        }
      )
    }
    ...
    disconnect (key, cb) {
      log('Disconnecting from: ', key || 'all connected swarms')
