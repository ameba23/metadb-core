const log = require('debug')('metadb-scan')
const fs = require('fs')
...
module.exports = class Shares extends EventEmitter {
  ...
  async scanDir (dir, opts = {}) {
    ...
    for await (const file of walk(dir, { ignorePatterns: this.ignorePatterns })) {
      ...
      if (dataParsed % MAXBATCH === 0) await this.pauseIndexing()
  ...
  async processFile (file, options = {}) {
    ...
    const metadata = await new Promise((resolve, reject) => {
      if (options.skipExtraction) return resolve({})
      log(
        `Extracting metadata from: ${file} length: ${readableBytes(size)} ${hash.slice(-4).toString('hex')}`
      )
      extract(fullPath, (err, metadata) => {
        if (err) {
          log(`Error from extractor: ${err.message} ${err.stack}`)
          return resolve({})
        }
        resolve(metadata)
      })
