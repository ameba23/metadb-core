...
// Handshake - ephemeral keys are exchanged, then public
// signing keys along with a fresh signature
...
module.exports = function (ourStaticKeypair, weAreInitiator, stream, swarmTopics, onPeer, callback) {
  ...
  const log = debug(`metadb-handshake ${ourStaticKeypair.publicKey.slice(0, 2).toString('hex')}`)
